...
class Log:
    ...
    @staticmethod
    def _dev_log(title, title_color, exc, data, objects, extra):
        try:
            if ENV.is_dev():
                traceback_string = extra.get("traceback", "")
                if traceback_string:
                    print(
                        f"##########TRACEBACK START##########\n{traceback_string}\n##########TRACEBACK END##########"
                    )
    ...
    def error(title, exc=None, data=None, objects=None):
        ...
        if ENV.is_dev():
            Log._dev_log(title, bcolors.FAIL, exc, data, objects, extra=extra)
    ...
    def warning(title, exc=None, data=None, objects=None):
        ...
        if ENV.is_dev():
            Log._dev_log(title, bcolors.WARNING, exc, data, objects, extra=extra)
            if config("SENTRY_LOCAL", cast=bool, default=False):
    ...
    def info(title, exc=None, data=None, objects=None):
        ...
        if ENV.is_dev():
            Log._dev_log(title, bcolors.OKBLUE, exc, data, objects, extra=extra)
            if config("SENTRY_LOCAL", cast=bool, default=False):
    ...
    def debug(title="", exc=None, data=None, objects=None):
        ...
        if ENV.is_dev():
            Log._dev_log(title, bcolors.MAGENTA, exc, data, objects, extra=extra)
            if config("SENTRY_LOCAL", cast=bool, default=False):
